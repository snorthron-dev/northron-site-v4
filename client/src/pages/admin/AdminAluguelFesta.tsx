...
// Status labels and colors for festa rentals
...
/**
 * Página administrativa para gerenciar solicitações de aluguel de cães para festas.
 * Lista todas as solicitações, permitindo contato e atualização de status. Mostra
 * dados como data do evento, horas contratadas, quantidade de cães e valor
 * estimado calculado pelo sistema. É acessível apenas para usuários
 * autenticados.
 */
export default function AdminAluguelFesta() {
  const utils = trpc.useUtils();
  const { data: alugueis, isLoading } = trpc.aluguelFesta.listar.useQuery({ limit: 100, offset: 0 });
  const atualizarMutation = trpc.aluguelFesta.atualizarStatus.useMutation({
    onSuccess: () => utils.aluguelFesta.listar.invalidate(),
    ...
    <AdminLayout>
      <div className="p-8">
        <div className="mb-8">
          ...
          <h1 className="text-white text-2xl font-bold">Aluguel para Festas</h1>
          <p className="text-white/40 text-sm mt-1">Solicitações de cães para eventos e festas</p>
        </div>
        {isLoading && (
                ...
                <div className="flex items-start justify-between gap-4 flex-wrap">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[#c9a84c] text-sm font-medium mb-1">
                      {aluguel.quantidadeCaes} cão/ães • {aluguel.horas}h • {aluguel.data}
                    </p>
                    ...
                    <div className="flex gap-4 mt-3 flex-wrap">
                      <a
                        href={"https://wa.me/" + aluguel.telefone.replace(/\D/g, "") + "?text=Olá " + aluguel.nome + "! Vi sua solicitação de aluguel para festa. Vamos conversar?"}
                        target="_blank"
